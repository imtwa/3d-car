...
const getThreeJs = async () => {
    ...
    new GLTFLoader().load(modelUrl.value, function (gltf) {
        ...
        carModel.traverse((obj) => {//遍历模型树结构，查询模型节点
            ...
            if (obj.name == '网格'
                || obj.name == '网格010' || obj.name == '网格001' || obj.name == '网格008'
                || obj.name == '网格019' || obj.name == '网格017') {
                obj.material = bodyMaterial
            }
            else if (obj.name == 'Grille') {
                obj.material = carbonMaterial
            }
            else if (obj.name == '网格_3' || obj.name == '网格_10'
                || obj.name == '网格017_3' || obj.name == '网格019_4'
                || obj.name == '网格008_3' || obj.name == '网格001_4'
                || obj.name == '网格010_3' || obj.name == 'RearDoor') {
                obj.material = chromeMaterial
            }
            else if (obj.name == 'DoorFrLeft') {
                doors1.push(obj)
                console.log(doors1)
            }
            else if (obj.name == 'DoorFrRight' || obj.name == 'DoorRearRight' || obj.name == 'DoorRearLeft') {
                doors2.push(obj)
                console.log(doors2)
            }
        })
    ...
    //打开车门
    function carOpen() {
        for (let index = 0; index < doors1.length; index++) {
            setAnimationDoor1({ y: 0 }, { y: -Math.PI / 4 }, doors1[index])
        }
        for (let index = 0; index < doors2.length; index++) {
            setAnimationDoor2({ z: Math.PI / 2 }, { z: Math.PI / 4 }, doors2[index])
        }
    }
    //关闭车门
    function carClose() {
        for (let index = 0; index < doors1.length; index++) {
            setAnimationDoor1({ y: -Math.PI / 4 }, { y: 0 }, doors1[index])
        }
        for (let index = 0; index < doors2.length; index++) {
            setAnimationDoor2({ z: Math.PI / 4 }, { z: Math.PI / 2 }, doors2[index])
        }
    }
    function setAnimationDoor1(start, end, mesh) {
    ...
    function setAnimationDoor2(start, end, mesh) {
        const tween = new TWEEN.Tween(start).to(end, 1000).easing(TWEEN.Easing.Quadratic.Out)
        tween.onUpdate((that) => {
    ...
    //车内第一视角
    function carFirstIn() {
        setAnimationCamera({ cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 }, { cx: 0, cy: 0.95, cz: -0.25, ox: 0, oy: 0.5, oz: 3 });
    }
    //车内第二视角
    ...
    //车外视角
    function carOut() {
        setAnimationCamera({ cx: 0, cy: 0.9, cz: 0.45, ox: 0, oy: 0.5, oz: 3 }, { cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 });
    }

...
const getThreeJs = async () => {
  ...
  new GLTFLoader().load(modelUrl.value, function (gltf) {
    ...
    carModel.traverse((obj) => {//遍历模型树结构，查询模型节点
      ...
      if (obj.name == 'body_19' || obj.name == 'Front_bumper_17' || obj.name == 'bonnet16'
        || obj.name == 'Door_left_08' || obj.name == 'Door_right_06'
        || obj.name == 'Rear_bumper_01' || obj.name == 'trunk_04' || obj.name == 'body_16' || obj.name == 'trunk_14'
        || obj.name == 'Door_right_23' || obj.name == 'Door_right_04' || obj.name == 'Door_left_11'
        || obj.name == 'Door_left_29') {
        obj.material = bodyMaterial
      }
      else if (obj.name == 'Front_bumper_09' || obj.name == 'Front_bumper_16' || obj.name == 'Front_bumper_24') {
        obj.material = carbonMaterial
      }
      else if (obj.name == 'Front_bumper_28' || obj.name == 'Front_bumper_20'
        || obj.name == 'Front_bumper_12' || obj.name == 'Rear_bumper_07'
        || obj.name == 'trunk_15' || obj.name == 'body_09') {
        obj.material = chromeMaterial
      }
      else if (obj.name == 'Door_left') {
        leftdoors.push(obj)
        console.log(leftdoors)
      }
      else if (obj.name == 'Door_right') {
        rightdoors.push(obj)
  ...
  function carOpen() {
    for (let index = 0; index < leftdoors.length; index++) {
      setAnimationDoor({ y: -Math.PI / 2 }, { y: THREE.MathUtils.degToRad(-145) }, leftdoors[index])
    }
    for (let index = 0; index < rightdoors.length; index++) {
      setAnimationDoor({ y: -Math.PI / 2 }, { y: -Math.PI / 4 }, rightdoors[index])
    }
  }
  //关闭车门
  function carClose() {
    for (let index = 0; index < leftdoors.length; index++) {
      setAnimationDoor({ y: THREE.MathUtils.degToRad(-145) }, { y: -Math.PI / 2 }, leftdoors[index])
    }
    for (let index = 0; index < rightdoors.length; index++) {
      setAnimationDoor({ y: -Math.PI / 4 }, { y: -Math.PI / 2 }, rightdoors[index])
    }
  }
  ...
  //车内第一视角
  function carFirstIn() {
    setAnimationCamera({ cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 }, { cx: 0, cy: 0.95, cz: -0.3, ox: 0, oy: 0.5, oz: 3 });
  }
  //车内第二视角
  function carSecondIn() {
    setAnimationCamera({ cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 }, { cx: 0, cy: 1, cz: -1.1, ox: 0, oy: 0.5, oz: 3 });
  }
  //车外视角
  function carOut() {
    setAnimationCamera({ cx: 0, cy: 0.95, cz: -0.3, ox: 0, oy: 0.5, oz: 3 }, { cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 });
  }

...
const getThreeJs = async () => {
    ...
    new GLTFLoader().load(modelUrl.value, function (gltf) {
        ...
        carModel.traverse((obj) => {//遍历模型树结构，查询模型节点
            ...
            if (obj.name == 'Aow_Benz_002-carpaint'
                || obj.name == 'Aow_Benz_01-carpaint' || obj.name == 'Aow_Benz_003-carpaint'
                || obj.name == 'Aow_Benz_004-carpaint' || obj.name == 'Aow_Benz_005-carpaint' || obj.name == 'Aow_Benz_002-black695'
                || obj.name == 'Aow_Benz_003-black695' || obj.name == 'Aow_Benz_004-black695' || obj.name == 'Aow_Benz_002-black50') {
                obj.material = bodyMaterial
            }
            else if (obj.name == '网格_3' || obj.name == '网格002_3'
                || obj.name == '网格004_3' || obj.name == '网格003_3' || obj.name == '网格005_3') {
    ...
    function carOpen() {
        for (let index = 0; index < leftdoors.length; index++) {
            setAnimationDoor({ y: 0 }, { y: -Math.PI / 4 }, leftdoors[index])
        }
        for (let index = 0; index < rightdoors.length; index++) {
            setAnimationDoor({ y: 0 }, { y: Math.PI / 4 }, rightdoors[index])
        }
    }
    //关闭车门
    function carClose() {
        for (let index = 0; index < leftdoors.length; index++) {
            setAnimationDoor({ y: -Math.PI / 4 }, { y: 0 }, leftdoors[index])
        }
        for (let index = 0; index < rightdoors.length; index++) {
            setAnimationDoor({ y: Math.PI / 4 }, { y: 0 }, rightdoors[index])
        }
    }
    ...
    //车内第一视角
    function carFirstIn() {
        setAnimationCamera({ cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 }, { cx: 0, cy: 1.1, cz: 0, ox: 0, oy: 0.5, oz: 3 });
    }
    //车内第二视角
    function carSecondIn() {
        setAnimationCamera({ cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 }, { cx: 0, cy: 1.2, cz: -1.2, ox: 0, oy: 0.5, oz: 3 });
    }
    //车外视角
    function carOut() {
        setAnimationCamera({ cx: 0, cy: 1.1, cz: 0, ox: 0, oy: 0.5, oz: 3 }, { cx: 2.25, cy: 1.4, cz: 4, ox: 0, oy: 0.5, oz: 0 });
    }

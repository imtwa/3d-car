  ...
  <a-card title="车型管理" :bordered="false">
    <a-form layout="inline" :model="queryParams">
      <a-form-item label="品牌">
        <a-select
          v-model:value="queryParams.brandId"
          placeholder="请选择品牌"
          style="width: 200px"
          allow-clear
        >
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'brandId'">
          {{ brandList.find((item) => item.value === record.brandId)?.label }}
        </template>
...
import {
  ...
  updateCarModel,
  deleteCarModel,
  changeCarModelStatus,
} from "@/api/system/car/car";
...
const handleStatusChange = async (record: any) => {
  try {
    await changeCarModelStatus({
      id: String(record.id), // 确保ID作为字符串传递
